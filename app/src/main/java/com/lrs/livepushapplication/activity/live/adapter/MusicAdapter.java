...
public class MusicAdapter extends RecyclerView.Adapter<MusicAdapter.MusicViewHolder> {
    ...
    public void onBindViewHolder(final MusicViewHolder holder, final int position) {
        ...
        if(mPosition == position) {
            ...
            holder.tvMusicName.setTextColor(mContext.getResources().getColor(R.color.green));
            if(position != 0) {
                holder.tvLoop.setVisibility(mIsLoop ? View.VISIBLE : View.GONE);
            }
        } else {
            holder.tvMusicCheck.setVisibility(View.INVISIBLE);
            holder.tvMusicName.setTextColor(mContext.getResources().getColor(R.color.black));
